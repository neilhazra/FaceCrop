...
for filename in os.listdir(directory): #for even file in the raw images folder
    ...
    if abs(float(rows/cols)-1.25) >0.03:    #if the image doesn't fit the specified aspect ratio throw it out
        skipped = skipped + 1
        print(rows/cols)
        continue
    if w*h < 72000:
    ...
    cv.imwrite("CroppedImages//" + filename[:-5] + '_cropped' + '.jpg', img)    #save the image
